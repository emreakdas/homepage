export default function StartHero() {
  return (
    <div className="mb-20">
      <span className="mb-4 block">Today</span>
      <p className="text-gray-800/70">
        Merhaba ben Emre! İstanbul'da yaşayan ve minimal tasarıma ilgi duyan bir
        ön uç geliştiricisiyim. Şuan IdeaSoft'da frontend developer olarak
        çalışıyorum. Öğrendiklerimi{" "}
        <a href="https://medium.com/@emreakdas" target="_blank" className="underline hover:text-gray-800 transition-all">
          medium
        </a>{" "}
        hesabımda paylaşıyor, açık kaynak projeler geliştirip{" "}
        <a href="https://github.com/emreakdas" target="_blank" className="underline hover:text-gray-800 transition-all">
          github
        </a>{" "}
        üzerinden yayınlıyorum.
      </p>
    </div>
  );
}